...
int sum(int root, int l, int r) {
  ...
  if (!v) {
    ...
    return sum(nodes[v].left, l, r);
  } if (nodes[v].key < l) {
    return sum(nodes[v].right, l, r);
  } if ((!nodes[v].left) && (!nodes[v].right)){
  return nodes[v].key;
  }
  if (nodes[v].min >= 1 && nodes[v].max <= r){
  return nodes[v].sum + nodes[v].key;
} return sum(nodes[v].left, l, r) + sum(nodes[v].right, l, r) + nodes[v].key;
}
