...
int specialMergeTriple(Triple t) {
  return merge(merge(t.middle, t.left), t.right);
}
...
int main(){
  ...
  cin >> n >> m;
  for (int i = 0; i < n; i++) {
    insert(root, nodesNumber, i);
    nodesNumber++;
  } for (int i = 0; i < m; i++){
    int l, r;
    cin >> l >> r;
    Triple splitted = getSubsegment(root, l, r);
    root = specialMergeTriple(splitted);
  } showTree(root);
  return 0;
}
